...
{if isset($p) AND $p}
	...
	{else}
		{assign var='requestPage' value=$plTools->getPaginationLinkBlog()}
	...
	{if $start!=$stop}
		...
		{else}
			<li id="pagination_previous" class="disabled"><span>&laquo;&nbsp;{l s='Previous' mod='plblog'}</span></li>
		{/if}
		{if $start>3}
		...
		{if $pages_nb > 1 AND $p != $pages_nb}
			...
			<li id="pagination_next"><a href="{$link->goPage($requestPage, $p_next)}">{l s='Next' mod='plblog'}&nbsp;&raquo;</a></li>
		{else}
			<li id="pagination_next" class="disabled"><span>{l s='Next' mod='plblog'}&nbsp;&raquo;</span></li>
		{/if}
		</ul>
	{/if}
	{if $nb_products > 10}
		<form action="" method="get" class="pagination">
			<p>
				...
				<input type="submit" class="button_mini" value="{l s='OK' mod='plblog'}" />
