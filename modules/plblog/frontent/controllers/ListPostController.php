<?php
class ListPostController extends FrontController
{
	...
	function setPath($name = null)
	{
		...
		else
		{
			...
			elseif (Tools::getValue('plidt'))
			{
				...
				$url = $link->getPageLink('blog/tag/'.Tools::getValue('pltn').'-'.Tools::getValue('plidt').'.html');
				// return $url;
			}
		}
	...
	function display()
	{
		...
		/* display title */
		$smarty->assign('pl_category_name', $this->getCategoryName(Tools::getValue('plidc')));
		/* -display title */
